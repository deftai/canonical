...
run_canon() {
    ...
    if [ -f "$REPO_ROOT/pnpm-workspace.yaml" ] && [ -f "$REPO_ROOT/packages/cli/dist/bin.js" ]; then
        node "$REPO_ROOT/packages/cli/dist/bin.js" "$@"
        return $?
    fi
    echo "canon pre-commit: cannot resolve the canon CLI." >&2
    echo "  Tried: \\$CANON_HOOKS_BIN, 'canon' on PATH, node packages/cli/dist/bin.js (pnpm-workspace.yaml present)." >&2
    echo "  Recovery: npm i -g @canonpack/cli, run 'pnpm run build' in this repo, or set CANON_HOOKS_BIN." >&2
    exit 2
}
