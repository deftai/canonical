...
- ! Production release order is fixed: rename `[Unreleased]` → `[X.Y.Z] - date`, land it via a release PR, tag `vX.Y.Z` ONLY after the merge, push the tag (publishes npm `@prod`). Promote to `latest`/`stable` after smoke. Optionally track the cut as a scope with `x-canonical/kind: release` and `x-canonical/version: X.Y.Z` (groups scopes delivered in that version; `render roadmap` surfaces a Releases section). ⊗ Tag plain `vX.Y.Z` without a changelog entry; ⊗ add a versioned entry without tagging.
