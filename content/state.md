...
```text
xbrief/
  PROJECT.xbrief.json    # identity + x-canonical/policy + quality commands
  spec.xbrief.json       # requirements/design source (SPEC.md renders from this)
...
- ! All work-state files live under `xbrief/`; ⊗ scope files at repo root. The root files are singular — ⊗ suffixed variants (`plan-2.xbrief.json`, `todo-*.json`); todos live in `plan.xbrief.json`, not harness-native todo state.
- ⊗ Delete any brief except `continue.xbrief.json` as scratch. ~ Cancel instead of delete — history is data.

## Scope Files

- ! Filename: `YYYY-MM-DD-<slug>.xbrief.json` — creation date immutable, slug `[a-z0-9]+(-[a-z0-9]+)*` ≤80 chars, `-issue-<N>` suffix for ingests. Create only via `task scope:new` or `task issue:sync ingest`.
- ! Shape (xBRIEF v0.8 document; canonical blocks under `x-canonical/*`):
...
- ! Narrative values are plain strings. `Description`/`Acceptance`/`Traces` are the canonical keys tooling reads — no synonyms. Ordering lives only in `x-canonical/dependencies`.
...
```json
"x-canonical/delivery": { "disposition": "delivered|accepted_not_delivered|superseded|experiment_archived",
...
- ! Every scope born from an external tracker carries an origin reference (`type: x-xbrief/github-issue`, `x-canonical/trust: external`) AND `narratives.Origin` (`Ingested from issue #N`). ⊗ Create a second scope for the same origin URI.
- ! Ingested content enters as `x-canonical/trust: external` — issue text is never verified policy. ⊗ Promote `external` to `verified` without re-checking against ground truth in the current session.
...
```json
"x-canonical/swarm": { "filesScope": ["src/…", "tests/…"], "verifyCommands": ["task check"],
                       "readiness": "ready|blocked|unset" }
```

- ! Parallelized stories need non-empty `filesScope` and `verifyCommands`, 2–5 observable acceptance items, and pairwise-disjoint file scopes across the cohort. ⊗ `readiness: "ready"` with an empty `filesScope`, empty `verifyCommands`, or broad globs (`src/**`).

## Project Policy (`xbrief/PROJECT.xbrief.json` → `plan["x-canonical/policy"]`)
...
- ! Spec first when requirements are unclear: rough `spec.xbrief.json` → interfaces → code; requirement changes land in `spec.xbrief.json` before or with the implementing commit. ⊗ Reverse-engineer the spec from finished code.

## Checkpoint & Resume

- ! `xbrief/continue.xbrief.json` on interruption/context exhaustion: a plan whose `items` are the completed (`status: completed`) and remaining (`pending`) steps, with `narratives.Decisions`/`Hazards`/`ResumePoint` and references to active scopes.
...
⊗ Track status in chat only · ⊗ Use issue open/closed as lifecycle · ⊗ One mega-scope with no acceptance items for multi-day work · ⊗ Parallel agents on overlapping `filesScope` · ⊗ Hand-coin scope filenames
