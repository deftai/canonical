...
- ! The envelope also instructs the worker to read `canonical.md` (and the load-when files its work needs) before acting, and carries the story's `file_scope`, `verify_commands`, and acceptance criteria.
...
- ! Before allocate, every story: `kind: story`, non-empty `swarm.file_scope` and `swarm.verify_commands`, 2–5 acceptance items, file scopes pairwise disjoint (or explicit single-owner serialization). `task swarm:run --stories` checks readiness and emits the worktree map + launch manifest — it does NOT spawn; you spawn via host APIs.
- ! One isolated worktree per parallel worker, from the validated map, created before any spawn. ⊗ Multiple implement leaves on the same repo root. ⊗ Overlapping `file_scope` without explicit serialization.
- ! Writes are fenced: a worker writes only inside its story's `file_scope`, further limited by `policy.runtimeAuthority.denyPaths` — deny always wins; there is exactly one fence definition.
...
- ! Anomalies from any worker (refusals, skipped acceptance criteria, diffs outside `file_scope`) lead your cohort summary — a worker writing outside its fence is a security finding, not a footnote.
