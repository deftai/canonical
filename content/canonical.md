...
| [feedback.md](./feedback.md) | Feedback / bug / feature ask, or `orient` shows metrics not prompted / not decided / expired; opt-in or opt-out of Canonical collection |
...
- ! On first mutation of a session: run `task orient` if present; else verify git status and that `xbrief/` is readable. If orient reports `metrics=not_prompted` (or not decided), load [feedback.md](./feedback.md) and follow its **User dialogue** for the one-time metrics consent offer before other mutation work. ⊗ Dump task recipes, verb names, or flag lists to the user — speak that file's plain-English dialogue only; keep **Agent actions** silent.
