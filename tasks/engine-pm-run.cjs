 ...
 * Lives under tasks/ so @canonpack/content prepack ships it beside
...
function executeAllowlisted(execFn, cmd, args, opts) {
  if (!WIN32_ALLOWLIST.has(cmd)) {
    throw new Error(`canon: allowlisted shim required, got ${JSON.stringify(cmd)}`);
  }
...
function runPackageScript(root, script, deps = {}) {
  ...
  if (!fs.existsSync(pkgPath)) {
    console.error(`canon: package.json missing at ${root}`);
    return 2;
  }
  let pkg;
  try {
    ...
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      console.error(`canon: package.json at ${root} is not a JSON object`);
      return 2;
    }
    pkg = parsed;
  } catch {
    console.error(`canon: package.json at ${root} is not valid JSON`);
    return 2;
  }
  if (!validateScriptName(script, pkg.scripts)) {
    console.error(`canon: package.json has no script ${JSON.stringify(script)}`);
    return 2;
  }

  const pinResult = parsePnpmPin(pkg.packageManager);
  if (!pinResult.ok) {
    console.error(`canon: invalid packageManager pin — ${pinResult.reason}`);
    return 3;
  }

  const envPm = String(process.env.CANON_PACKAGE_MANAGER || "")
    .trim()
    .toLowerCase();
  if (envPm === "npm") {
    try {
      ...
      console.error(`canon: npm run ${JSON.stringify(script)} failed`);
      return 1;
    }
  }
  ...
  console.error(`canon: neither pnpm nor corepack is available to run ${JSON.stringify(script)}.`);
  if (pinResult.pin) {
    console.error(
  ...
  console.error("  Or set CANON_PACKAGE_MANAGER=npm for an explicit npm build path.");
  return 127;
}
