 ...
 *
 * Exit codes:
 *   0 — dist is fresh enough (or CANON_SKIP_TS_BUILD=1); skip rebuild
 *   1 — rebuild required (missing dist, newer sources, or CANON_FORCE_TS_BUILD=1)
...
function main() {
  ...
  if (!root) {
    console.error("canon: ts-build-fresh usage: ts-build-fresh.cjs <CANON_ROOT>");
    process.exit(2);
  }
  if (process.env.CANON_FORCE_TS_BUILD === "1") {
    process.exit(1);
  }
  if (process.env.CANON_SKIP_TS_BUILD === "1") {
    process.exit(0);
  }
